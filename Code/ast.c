...
void ast_init(char nt_string[NUM_OF_NONTERMINALS][MAX_SYMBOL_LENGTH]){
	for(int i = 0; i < NUM_OF_NONTERMINALS; i++)
		strcpy(non_terminal_string[i], nt_string[i]);
	
	// int num_of_imp_tokens;	
	// important_terminals = (token_name*)malloc(sizeof(token_name) * num_of_imp_tokens);
}
...
bool is_base_case(tree_node *node) {
  ...
  if(node == NULL)
	return false;
  if (node->sym.is_terminal == true || node->parent == NULL) {	//A -> a me a and S->.. me S
    return false;
  }

  if ((rchild && rchild->sym.is_terminal == false) && node->sym.nt == rchild->sym.nt)		// A -> BA1 me A
    return false;
  ...
  if (node->sym.nt == node->parent->sym.nt) {		// A -> B A1 me A1, if it reaches here it means A1 is base case for A
    return true;
  }
...
void extend_inh_node(tree_node *node1, tree_node *node2) {
	...
	if (node1->node_inh == NULL) {		// list doesn't exist already, create a node with label
...
tree_node *construct_ast(tree_node *parse_tree_root) {
	...
	do {
		...
		if (temp->visited == false) {
			...
			if ((is_recursion(temp) == true) && (temp->parent->sym.nt == temp->sym.nt || ends_in_dash(non_terminal_string[temp->sym.nt]) == true)) {
			...
			if (is_recursion(temp) == false) {
				if(lchild != NULL && (lchild == rchild) ){
					...
					if (lchild->sym.is_terminal == false) {	// It's a chain of nts, simple pass synthesized nodes A->b => A.node_syn = B.node_syn
						temp->node_syn = lchild->node_syn;
					}

					else {
						temp->node_syn = copy_node(lchild);	// MD -> id type rule, initialize syn_attr of MD
					}
				} 
				...
				if (is_base_case(temp) == false) {	//returning from recursion , do A->BA1, A1.node_syn = A1.node_inh and A.node_syn = A1.node_syn
			...
			while (child != NULL) {
				...
				child = delete_child(temp, NULL, child);	//free all children, no parse tree node is preserved to avoid nullifying parents/siblings fields 
